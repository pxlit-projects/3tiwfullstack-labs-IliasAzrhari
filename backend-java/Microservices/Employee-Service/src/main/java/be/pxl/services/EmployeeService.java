...
import be.pxl.repository.EmployeeRespository;
...
public class EmployeeService implements IEmployeeService {
    ...
    public List<EmployeeResponse> findByDepartment(Long departmentId) {
        return employeeRespository.findByDepartmentId(departmentId)
                .stream().map(employee -> mapToEmployeeResponse(employee)).toList();
    }

    @Override
    public List<EmployeeResponse> findByOrganization(Long organizationId) {
        return employeeRespository.findByOrganizationId(organizationId)
                .stream().map(employee -> mapToEmployeeResponse(employee)).toList();
    }
